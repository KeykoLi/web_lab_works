...
function displayObjectsOnPage(objects) {
    ...
    objects.forEach((object) => {
        ...
        const editButton = displayElement.querySelector('.edit_button');
        editButton.addEventListener('click', () => {
        // Перенаправлення на edit_page.html та передача ідентифікатора об'єкта
        window.location.href = `/edit_page.html?id=${object.id}`;
    });
...
async function deleteObject(id) {
    const res = await fetch(`http://localhost:5503/api/complex/${id}`, {
        method: 'DELETE',
        headers: {
